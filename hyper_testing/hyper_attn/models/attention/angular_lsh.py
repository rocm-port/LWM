...
class AngularLSH(torch.nn.Module):
    ...
    def __init__(self, num_projs, dim, rng=None, dtype = torch.float32):
        super().__init__()
        self.num_projs = num_projs

        if num_projs > 0:
            if (rng is not None):
                proj_dir = rng.normal_torch(dim + (num_projs,), dtype=dtype)
            else:
                proj_dir = torch.randn(dim + (num_projs,), dtype=dtype)
    ...
    def hash(self, mat):
        ...
        mask =  torch.matmul(mat, self.proj_dir)
        mask = mask > 0
