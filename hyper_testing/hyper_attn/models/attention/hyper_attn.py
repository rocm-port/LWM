...
class HyperAttention(torch.nn.Module):
    ...
    def __init__(self, input_dim=64, lsh_num_projs=7, block_size=256, sample_size=256, min_seq_len=4096, cuda=False, dtype=torch.float32, rng=None):
        super().__init__()
        ...
        self.cuda = cuda
        self.rng = rng
        self.lsh = AngularLSH(num_projs=self.lsh_num_projs, dim=(1, 1, input_dim), dtype=dtype, rng=rng)
    ...
    def forward_no_causal_mask(self, query, key, value, scale):
        ...
        if sample_size > 0 and (n_query > query_block_size) and (n_key > key_block_size):
            rand_size=(batch_size, head_size, sample_size)
            if (self.rng is not None):
                sampled_set = self.rng.randint_torch(rand_size, n_key, query_sorted.device, dtype=torch.int64)
            else:
